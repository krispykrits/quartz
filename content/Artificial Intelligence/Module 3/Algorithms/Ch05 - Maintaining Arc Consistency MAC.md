...
**Source:** slides/pages 39.

## Purpose
After each decision, seed AC-3 with arcs from unassigned neighbors into the assigned variable; propagate any resulting deletions further.
...
## Lecture Procedure
The steps here follow the lecture's algorithmic description on slides 39. See [[Lectures/Ch05 - Constraint Satisfaction Problems|Main Lecture Note]] for the source-order explanation and examples.
...
```python
def mac_after_assignment(var, domains, neighbors, assignment, constraint, ac3_fn):
    initial = [(n, var) for n in neighbors[var] if n not in assignment]
    return ac3_fn(domains, neighbors, constraint, initial_arcs=initial)
```
